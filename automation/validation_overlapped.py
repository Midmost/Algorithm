...
########################################################################
# def find_overlapping_label(find_list):
#     # 리스트의 중복요소만 리턴
#     # 입력: find_list
#     # 출력: 리스트의 중복요소
#     list_count = {}
...
#         if v >= 2:
#             overlapped_list.append(k)
#     return overlapped_list

# find_list = ['temporal_artery', 'temporal_artery', 'df']
# print('overlappp', find_overlapping_label(find_list))
########################################################################


def isOverlapped (find_list):
    isOverlapped = len(find_list) - len(set(find_list))
    if isOverlapped > 0:
        return True
...
scene_file_paths = get_all_scenes("Z:/Dataset/3dSlicer/HeadCT/alkee-testing") # Reviewing 
...
for scene_file_path in scene_file_paths:
    ...
    try:
        ...
        invalid_labels = get_invalid_labels()
        if len(invalid_labels) != 0 or isOverlapped(find_label()):
            # 씬은 이미 불러와져 있고
